...
# =============================================================================
"""
DMAC SoC for Arty A7-100T
===========================
Builds a LiteX SoC with:
  - No CPU (UARTBone for register access)
  - UART at 115200 baud
  - DMAC IP (1 channel, loopback SRAM)
  - LED0 = ch0_busy, LED1 = irq
...
class DMACSoC(SoCCore):
    def __init__(self, sys_clk_freq: float = 100e6, **kwargs):
        ...
        SoCCore.__init__(self, platform,
            ...
            ident    = "DMAC IP Test SoC - Arty A7-100T",
            **kwargs
        )
        ...
        # ── DMAC IP ───────────────────────────────────────────────────────
        self.submodules.dmac = DMAC(platform)
        self.add_csr("dmac")
        ...
        self.add_csr("leds")
        self.comb += [
            leds[0].eq(self.dmac.status.status[0]),  # busy
            leds[1].eq(self.dmac.irq),                # irq
        ]


# ── Main ─────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="DMAC SoC on Arty A7-100T")
    builder_args(parser)
    ...
    soc = DMACSoC(**soc_core_argdict(args))

...
module dmac_wb #(
  ...
  // ── Combinational address decode (synthesis-safe) ──────────────────────
  logic [7:0] wb_waddr, wb_ch_off, wb_rd_ch_off;
  logic [2:0] wb_ch_idx, wb_rd_ch_idx;
  always_comb begin
    wb_waddr      = ADR_I[9:2];
    wb_ch_off     = wb_waddr - 8'h10;
    wb_ch_idx     = wb_ch_off[5:3];
    wb_rd_ch_off  = ADR_I[9:2] - 8'h10;
    wb_rd_ch_idx  = wb_rd_ch_off[5:3];
  end
  ...
  always_ff @(posedge CLK_I) begin
    if (RST_I) begin
      ...
      if (CYC_I && STB_I && !ACK_O) begin
        ...
        if (WE_I) begin
          // ── Write ────────────────────────────────────────────────────
          if (wb_waddr == 8'h00) begin  // GLOBAL_CTRL
            reg_global_enable <= DAT_I[0];
          end
          else if (wb_waddr == 8'h02) begin  // GLOBAL_IRQ (W1C)
            reg_irq_status <= reg_irq_status & ~DAT_I;
          end
          else if (wb_waddr >= 8'h10) begin  // Per-channel
            
            
            if (int'(wb_ch_idx) < NUM_CH) begin
              case (wb_ch_off[2:0])
                3'h0: begin  // CH_CTRL
                  reg_ch_ctrl[wb_ch_idx] <= DAT_I;
                  if (DAT_I[CTRL_ENABLE])
                    ch_start[wb_ch_idx] <= 1'b1;
                end
                3'h1: ;  // CH_STATUS W1C (handled in irq_status)
                3'h2: reg_ch_src[wb_ch_idx]   <= DAT_I;
                3'h3: reg_ch_dst[wb_ch_idx]   <= DAT_I;
                3'h4: reg_ch_count[wb_ch_idx] <= DAT_I[15:0];
                3'h5: reg_ch_ndesc[wb_ch_idx] <= DAT_I;
                default: ;
              endcase
            end
          end
          ...
          else if (wb_waddr == 8'h01)
            DAT_O <= {{(32-NUM_CH){1'b0}}, ch_busy};
          else if (wb_waddr == 8'h02)
            DAT_O <= reg_irq_status;
          else if (wb_waddr == 8'h03)
            DAT_O <= core_version;
          else if (wb_waddr >= 8'h10) begin
            wb_rd_ch_off = wb_waddr - 8'h10;
            wb_rd_ch_idx = wb_rd_ch_off[5:3];
            if (int'(wb_rd_ch_idx) < NUM_CH) begin
              case (wb_rd_ch_off[2:0])
                3'h0: DAT_O <= reg_ch_ctrl[wb_rd_ch_idx];
                3'h1: DAT_O <= {28'b0, ch_busy[wb_rd_ch_idx],
                                ch_te[wb_rd_ch_idx], ch_ht[wb_rd_ch_idx],
                                ch_tc[wb_rd_ch_idx]};
                3'h2: DAT_O <= reg_ch_src[wb_rd_ch_idx];
                3'h3: DAT_O <= reg_ch_dst[wb_rd_ch_idx];
                3'h4: DAT_O <= {16'b0, reg_ch_count[wb_rd_ch_idx]};
                3'h5: DAT_O <= reg_ch_ndesc[wb_rd_ch_idx];
                3'h6: DAT_O <= 32'h0;
                3'h7: DAT_O <= {16'b0, ch_remain[wb_rd_ch_idx]};
